...
The band layout is CSS **multi-column** (`columns-1 sm:columns-2` + `break-inside-avoid`), not a grid. Card heights vary by roughly 4x between a scan and a name-only entry; a grid left large dead gaps, and reserving an empty media panel on every card to equalise rows looked worse still (Heena flagged both). Columns pack and self-balance. Order is column-major as a result, which is fine for an unordered credential list — don't switch back to `grid` to "fix" the ordering. It has its own section (`site/certifications.tsx`, index `05`) rather than a card inside Skills. A `kind` group with no entries is dropped, so the volunteering band stays hidden until it has content, and the whole section returns `null` if there are none at all. Several seeded issuers are deliberately blank because they weren't stated on Heena's LinkedIn — leave them empty rather than guessing which platform issued them.
